...
const Menu: React.FC<MenuProps> = ({
  ...
  const mouseEnter = (el: Element, id: number) => {
    ...
    const hoverStart = gsap.timeline({
      onUpdate: () => {
        if (feDisplacementMapRefs[id] && feDisplacementMapRefs[id].current) {
          feDisplacementMapRefs[id].current.scale.baseVal = scale.scale;
        }
      },
    });
    hoverStart
  ...
  return (
    <div id="menu" ref={menuRef} className={S.menu}>
      ...
      <Close id="closeSVG" className={S.close} onClick={() => setOpen(false)} />
      <Link to="/">
        <p
          data-filter="0"
          data-splitting=""
      ...
      <Link to="/contact">
        <p
          ...
          className={`${S.menuItem} split-text menuItem`}
        >
          Contacto <Icon className={S.icon} />
        </p>
      </Link>
    </div>
  );
